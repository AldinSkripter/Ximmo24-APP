...
class NotificationService {
  ...
  // Initialize notification services (channels + Firebase listeners only).
  // Does NOT prompt the user. Call [requestPermissions] from the home screen
  // so the OS popup doesn't appear during splash/onboarding.
  static Future<void> init(BuildContext context) async {
    await _initializeAwesomeNotifications();
    await _registerFirebaseListeners(context);
  }

  // Prompt for notification permission. Safe to call multiple times — only
  // shows the OS popup once per install.
  static Future<void> requestPermissions() async {
    ...
    if (!isAllowed && !alreadyAsked) {
      ...
      await _awesomeNotifications.requestPermissionToSendNotifications(
        channelKey: AppConfig.notificationChannel,
        permissions: [
  ...
  static Future<void> _handleNotificationNavigation(
    ...
    } else if (type == 'agent_verification' || type == 'user_verification') {
      if (context != null) {
  ...
  static Future<void> _navigateToChatScreen(
    ...
    final context = Constant.navigatorKey.currentContext;
    if (context != null) {
      if (payload['role_context']?.toString() == 'agent') {
        ...
        CupertinoPageRoute<dynamic>(
          builder: (context) => MultiBlocProvider(
            ...
            child: ChatScreenNew(
              ...
              isAgentVerified:
                  payload['is_agent_verified']?.toString() == 'true',
