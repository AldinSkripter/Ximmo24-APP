...
abstract class CloudState<T extends StatefulWidget> extends State<T> {
  ...
  void appendToList<E>(String key, E value, {bool? disableClone}) {
    if (!cloudData.containsKey(key)) {
      cloudData[key] = [value];
    }
    if (cloudData[key] is List<E>) {
      if (disableClone ?? false) {
        if (!(cloudData[key] as List<E>).contains(value)) {
          (cloudData[key] as List<E>).add(value);
          notify(key, value);
        }
      } else {
        (cloudData[key] as List<E>).add(value);
  ...
  void appendToListWhere<E>({
    ...
    if (cloudData[listKey] is List<Map<String, dynamic>>) {
      ...
      if (disableClone != true ||
        ...
        final indexWhere = list.indexWhere(
          (element) => element[whereKey] == equals,
        );
        if (indexWhere >= 0) {
          list[indexWhere] = add;
        } else {
  ...
  void toGroup(String groupName, dynamic key, dynamic value) {
