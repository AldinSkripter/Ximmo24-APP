...
class _CurrentPackageTileCardState extends State<CurrentPackageTileCard> {
  ...
  // Other (non listing/featured) feature ids. Watermark (8) is agent-only.
  List<int> get _otherFeatureIds =>
      ActiveRoleManager.isAgent ? const [5, 6, 7, 8] : const [5, 6, 7];
  ...
  Widget build(BuildContext context) {
    ...
    final isOtherFeaturesNotAvailable = _otherFeatureIds.every(
      (id) => _getFeatureById(id) == null,
    );
  ...
  Widget buildPackageFeatures({
    ...
    // Filter to only include "other" features (watermark is agent-only)
    var filteredFeatures = allPackageFeatures
        .where(
          (feature) => _otherFeatureIds.contains(feature.id),
        )
        .toList();
