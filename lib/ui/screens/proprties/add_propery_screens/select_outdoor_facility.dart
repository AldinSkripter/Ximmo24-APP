...
class _SelectOutdoorFacilityState extends State<SelectOutdoorFacility> {
  ...
  void initState() {
    ...
    // Pre-fill from existing assigned facilities (edit mode)
    if (widget.apiParameters?['isUpdate'] as bool? ?? false) {
      final updatedList = List<int>.from(_selectedIdsList.value);
      for (final facility in facilities) {
        final id = int.tryParse(facility.facilityId ?? '');
        if (id != null) {
          if (!updatedList.contains(id)) {
            updatedList.add(id);
          }
          if (!distanceFieldList.keys.contains(id)) {
            distanceFieldList[id] = TextEditingController(
              text: facility.distance.toString(),
            );
          }
        }
      }
  ...
  OutdoorFacility? getSelectedFacility(int id) {
    for (final element in facilityList) {
      if (element.id == id) {
        return element;
      }
    }
      ...
      child: BlocListener<CreatePropertyCubit, CreatePropertyState>(
        listener: (context, state) async {
          ...
          if (state is CreatePropertyFailure ||
              state is CreatePropertySuccess) {
            setState(() {
                      ...
                      Expanded(
                        child: UiUtils.buildButton(
                          ...
                          onPressed: () async {
                            ...
                            parameters?.addAll({'is_draft': true});
                          ...
                          return UiUtils.buildButton(
                            ...
                            onPressed: () async {
                              ...
                              final isPackageAvailable =
                                  await _checkPackageCubit.checkAvailability(
