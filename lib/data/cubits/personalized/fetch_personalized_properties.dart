  ...
  Future<void> fetch({
    ...
    try {
      if (forceRefresh ?? false) {
        final result = await _propertyRepository.getPersonalizedProeprties(
          offset: 0,
        ...
        if (state is! FetchPersonalizedPropertySuccess) {
          ...
          await CheckInternet.check(
            onInternet: () async {
              final result = await _propertyRepository
                  .getPersonalizedProeprties(
                    offset: 0,
                    searchQuery: _lastSearchQuery.isEmpty
                        ? null
                        : _lastSearchQuery,
                  );
  ...
  Future<void> fetchMore() async {
    ...
    try {
      if (state is FetchPersonalizedPropertySuccess) {
        ...
        emit(
          (state as FetchPersonalizedPropertySuccess).copyWith(
            isLoadingMore: true,
          ),
        );
        final result = await _propertyRepository.getPersonalizedProeprties(
