...
class FetchMostViewedPropertiesCubit
        ...
        await Future<dynamic>.delayed(
          Duration(
            seconds: loadWithoutDelay ?? false
                ? 0
                : AppConfig.hiddenAPIProcessDelay,
          ),
        );
        // });
      } else {
  ...
  void update(PropertyModel model) {
    if (state is FetchMostViewedPropertiesSuccess) {
      ...
      emit(
        (state as FetchMostViewedPropertiesSuccess).copyWith(
  ...
  Future<void> fetchWithFilter(FilterApply? filter, String searchQuery) async {
    // Always lock in the most-viewed flag regardless of what filter provides,
    // so the initial load matches the most-viewed data set used by fetchMore.
    final merged = (filter ?? FilterApply()).copy()
      ..addOrUpdate(
        const FlagsFilter(promoted: false, premium: false, mostViewed: true),
      );
    try {
      emit(FetchMostViewedPropertiesInProgress());
