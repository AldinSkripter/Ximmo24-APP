...
class FetchMostLikedPropertiesCubit extends Cubit<FetchMostLikedPropertiesState>
  ...
  void update(PropertyModel model) {
    if (state is FetchMostLikedPropertiesSuccess) {
      ...
      emit(
        (state as FetchMostLikedPropertiesSuccess).copyWith(
  ...
  Future<void> fetchWithFilter(FilterApply? filter, String searchQuery) async {
    // Always lock in the most-liked flag regardless of what filter provides,
    // so the initial load matches the most-liked data set used by fetchMore.
    final merged = (filter ?? FilterApply()).copy()
      ..addOrUpdate(
        const FlagsFilter(promoted: false, premium: false, mostLiked: true),
      );
    try {
      emit(FetchMostLikedPropertiesInProgress());
